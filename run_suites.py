...
default_config = {
    ...
    "aiob" : {  #aicb workload suites with computation
        ...
        "llama7B_zero3_aiob": 0,
        "llama65B_zero2_aiob": 0,
...
def read_config(config):
    ...
    if int(ds_conf["llama7b_zero3"]):
        running_command["deepspeed3_llama13b"] = (
            f"bash scripts/deepspeed_llama.sh --zero_stage 3 -m 13 --epoch_num 10  \
    ...
    if int(ds_conf["llama65b_zero2"]):
        running_command["deepspeed2_llama65b"] = (
            f"bash scripts/deepspeed_llama.sh --zero_stage 2 -m 65 --epoch_num 10  \
    ...
    if int(megatron_conf["llama_7B"]):
        running_command["megatron_llama7B"] = (
            f"bash scripts/megatron_gpt.sh -m 7 --tensor_model_parallel_size 1 --epoch_num 10  "
        )
    if int(megatron_conf["gpt_13B_tp"]):
        running_command["megatron_gpt13b_tp"] = (
            f"bash scripts/megatron_gpt.sh -m 13 --tensor_model_parallel_size 4 --epoch_num 10  "
        )
    if int(megatron_conf["gpt_13B_sp"]):
        running_command["megatron_gpt13b_sp"] = (
            f"bash scripts/megatron_gpt.sh -m 13 --tensor_model_parallel_size 4 --epoch_num 10 --sp"
        )
    ...
    if int(megatron_conf["gpt_22B"]):
        running_command["megatron_gpt_22B"] = (
            f"bash scripts/megatron_gpt.sh -m 22 --tensor_model_parallel_size 8 --epoch_num 10 "
        )
    ...
    if int(aiob_conf["gpt_13B_tp_aiob"]):
        running_command["megatron_gpt13b_tp_aiob"] = (
            f"bash scripts/megatron_gpt.sh -m 13 --tensor_model_parallel_size 4 --epoch_num 10 --aiob_enable "
        )
    ...
    if int(aiob_conf["gpt_22B_aiob"]):
        running_command["megatron_gpt_22B_aiob"] = (
            f"bash scripts/megatron_gpt.sh -m 22 --tensor_model_parallel_size 8 --epoch_num 10 --aiob_enable "
        )
    ...
    if int(aiob_conf["llama7B_zero3_aiob"]):
        running_command["deepspeed3_llama7b_aiob"] = (
            f"bash scripts/deepspeed_llama.sh --zero_stage 3 -m 7 --epoch_num 10  \
      --reduce_bucket_size 5000000000 --allgather_bucket_size 5000000000 \
      --param_persistence_threshold 40960 --aiob_enable" 
        )
    if int(aiob_conf["llama65B_zero2_aiob"]):
        running_command["deepspeed2_llama65b_aiob"] = (
            f"bash scripts/deepspeed_llama.sh --zero_stage 2 -m 65 --epoch_num 10  \
          --reduce_bucket_size 1000000000 --allgather_bucket_size 500000000 \
